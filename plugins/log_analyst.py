...
async def _log_analyst(ult):
    ...
    try:
        ...
        telegraph_content = (
            f"<h2>Log Analysis Report</h2>"
            f"<b>File:</b> <code>{file_name}</code><br/>"
            f"<b>Type:</b> {log_type}<br/>"
            f"<b>Date:</b> {time.ctime()}<br/><br/>"
            f"<h3>AI Analysis</h3>\n{analysis}\n\n"
            f"<hr/>"
            f"<small>Generated by Ultroid AI Analyst | {duration}s | {usage} tokens</small>"
        )
